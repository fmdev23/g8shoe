...
<span>Đăng xuất</span>
    ...
    <script>
        document.getElementById('<%= fuImage.ClientID %>').addEventListener('change', function (event) {
            const imgPreview = document.createElement("img");
            imgPreview.src = URL.createObjectURL(event.target.files[0]);
            imgPreview.style.width = "100px";
            document.querySelector('.form-group label[for="fuImage"]').appendChild(imgPreview);
        });
