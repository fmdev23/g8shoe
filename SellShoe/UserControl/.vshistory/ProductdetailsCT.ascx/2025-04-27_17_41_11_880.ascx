...
<section id="prodetails" class="section-p1">
    <%
        ...
        if (product != null)
        {
            var category = listDMSP.FirstOrDefault(c => c.id == product.ProductCategoryId);
    %>

    <!-- Hình ảnh sản phẩm -->
    <div class="single-pro-image">
        <img src="<%= product.Image %>" width="100%" id="MainImg" alt="Ảnh chính">
        <div class="small-img-group">
            <% for (int i = 0; i < 3; i++)
                { %>
            <div class="small-img-col">
                <img src="<%= product.Image %>" width="100%" class="small-img" alt="Ảnh nhỏ">
            </div>
            <% } %>
        </div>
    </div>
    ...
    <div class="single-pro-details">
        <div class="breadcrumbs">
            <span>Trang chủ</span> &gt;
        <span><%= category != null ? category.Title : "" %></span> &gt;
        ...
        <div class="price-box">
            ...
            <span class="new-price"><%= product.PriceSale %>đ</span>
            <% }
            else
            { %>
