...
<section id="product1" class="section-p1">
    ...
    <div class="filter-tabs">
        ...
        <% foreach (var item in categoryMap)
            {
                var categoryName = db.tb_ProductCategories.FirstOrDefault(c => c.Id == item.Key)?.Name ?? "Unknown";
        %>
        <button class="filter-tab" data-category="<%= item.Value %>"><%= categoryName.ToUpper() %></button>
        <% } %>
    </div>
