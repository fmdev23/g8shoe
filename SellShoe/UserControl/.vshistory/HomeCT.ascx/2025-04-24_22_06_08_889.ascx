...
<section id="product1" class="section-p1">
    ...
    <div class="filter-tabs">
        ...
        <% foreach (var item in categoryMap)
            {
                string name = db.tb_ProductCategories.FirstOrDefault(c => c.Id == item.Key)?.Name;
        %>
        <button class="filter-tab" data-category="<%= item.Value %>"><%= name.ToUpper() %></button>
        <% } %>
    </div>
    ...
    <div class="pro-container">
        ...
        <div class="pro" data-category="<%= categoryMap[listSP[i].CategoryId ?? 0] %>">
            <img src='<%="../img/products/" + listSP[i].Image %>' alt="">
            <div class="des">
