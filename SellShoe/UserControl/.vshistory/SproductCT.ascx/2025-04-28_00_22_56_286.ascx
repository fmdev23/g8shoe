            ...
            <section class="review-section">
                <div class="review-container">
                    ...
                    <div class="review-form">
                        ...
                        <input type="email" id="reviewerEmail" placeholder="Email của bạn" />
                        ...
                        <textarea id="reviewText" placeholder="Đánh giá của bạn" rows="4" cols="50"></textarea>
                        <button id="submitReviewBtn" class="submit-btn">Gửi đánh giá</button>
                    </div>
                </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('submitReviewBtn').addEventListener('click', function () {
            ...
            var productId = CURRENT_PRODUCT_ID; // nhớ gán đúng product id nhé

            fetch('/SubmitReview.aspx/AddReview', { // <-- Sửa url đúng WebMethod bạn viết nhé
                method: 'POST',
                headers: {
                ...
                .then(data => {
                    if (data.d) { // WebMethod trả về, cần đọc .d
                        alert('Cảm ơn bạn đã gửi đánh giá!');
                        addReviewToPage(reviewerName, selectedRating, reviewText);
                        clearReviewForm();
                    } else {
        ...
        function addReviewToPage(name, rating, text) {
            var reviewList = document.getElementById('reviewList'); // <-- chỗ bạn hiển thị list đánh giá
            if (!reviewList) {
                console.warn('Chưa có container #reviewList để append đánh giá!');
                return;
            }

            var reviewItem = document.createElement('div');
            reviewItem.className = 'review-item'; // class tùy bạn style

            var starsHtml = '';
            for (var i = 1; i <= 5; i++) {
                if (i <= rating) {
                    starsHtml += '<i class="fas fa-star"></i>'; // full sao
                } else {
                    starsHtml += '<i class="far fa-star"></i>'; // sao trống
                }
            }

            reviewItem.innerHTML = `
            <div class="review-header">
                <strong>${name}</strong> - ${starsHtml}
            </div>
        ...
        `;
            ...
            reviewList.prepend(reviewItem); // thêm đánh giá mới lên đầu danh sách
        }
