...
<!-- xử lý rating và kiểm tra đăng nhập khi gửi đánh giá -->
<script>
    ...
    // Kiểm tra đăng nhập khi click nút gửi đánh giá (client-side check trước)
    if (submitReviewBtn) {
        submitReviewBtn.addEventListener('click', function(e) {
            ...
            const reviewerName = document.getElementById('<%= txtReviewerName.ClientID %>').value.trim();
            const rating = ratingField.value;

            if (!reviewerName || !rating || rating == '0') {
                e.preventDefault();
                Swal.fire({
                    icon: 'warning',
                    title: 'Thông tin chưa đầy đủ!',
                    text: 'Vui lòng nhập tên và chọn số sao đánh giá.',
